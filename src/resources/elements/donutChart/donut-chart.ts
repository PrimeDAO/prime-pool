import { Pool } from "./../../../entities/pool";
import { bindable } from "aurelia-framework";
import * as d3 from "d3";
import "./donut-chart.scss";

export class DonutChart {


  @bindable pool: Pool;

  attached() {
    this.data = this.pool.assetTokensArray;
    this.render();
  }

  data:any= [
    {
      "Amount": "45%",
      "Value": 100,
      "Price": "$12.2",
      "Change": " 14%",
      "image": "images/ada.png",
    },
    {
      "Amount": "22%",
      "Value": 44,
      "Price": "$10.1",
      "Change": "21%",
      "image": "images/block.png",
    },
    {
      "Amount": "67%",
      "Value": 215,
      "Price": "$2.2",
      "Change": "16%",
      "image": "images/cc.png",
    },
    {
      "Amount": "53%",
      "Value": 285,
      "Price": "$31.5",
      "Change": "32%",
      "image": "images/cny.png",
    },
    {
      "Amount": "12%",
      "Value": 164,
      "Price": "$22.4",
      "Change": "5%",
      "image": "images/elix.png",
    },
  ];

  render () {


    const width = 300;
    const height = 300;
    const radius = Math.min(width, height) / 2;


    const divNode = d3.select("body").node();

    const outerRadius:any = 145;

    const color = d3.scale.ordinal()
      .range(["FF495B", "#8668FC", "#1EE0FC", "#95D86E", "#FAA04A"]);

    const arc = d3.svg.arc()
      .padRadius(outerRadius)
      .innerRadius(radius * 0.55);

    const pie = d3.layout.pie()
      .sort(null)
      .padAngle(0.03)
      .value(function(d:any) { return d.Value; });

    d3.select("#chart").append("div")
      .attr("id", "mainPie")
      .attr("class", "pieBox");

    const svg = d3.select("#mainPie").append("svg")
      .attr("width", width)
      .attr("height", height);

    var g:any = svg.append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");

    const defs = svg.append("defs");
    const filter = defs.append("filter")
      .attr("id", "drop-shadow")
      .attr("height", "130%");

    filter.append("feGaussianBlur")
      .attr("in", "SourceAlpha")
      .attr("stdDeviation", 0.1)
      .attr("result", "blur");

    filter.append("feOffset")
      .attr("in", "blur")
      .attr("dx", 0)
      .attr("dy", 0)
      .attr("result", "offsetBlur");

    const feMerge = filter.append("feMerge");

    feMerge.append("feMergeNode")
      .attr("in", "offsetBlur");
    feMerge.append("feMergeNode")
      .attr("in", "SourceGraphic");

    const pattern = defs.append("pattern")
      .attr("id", "image")
      .attr("width", 5)
      .attr("height", 5);

    const image = pattern.append("image").attr("x", 48).attr("y", 10).attr("width", 40).attr("height", 40);
    const text= pattern.append("text").attr("x", 38).attr("y", 0).attr("width", 40).attr("height", 40);


    const centerSvg = d3.select("#mainPie svg").append("circle")
      .attr("class", "image")
      .attr("fill", "#975EEE")
      .attr("r", "68")
      .attr("cx", 62).attr("cy", 62)
      .attr("transform", "translate(88, 88)");

    var g = g.selectAll(".arc")
      .data(pie(this.data))
      .enter().append("g")
      .attr("class", "arc")
      .each(function(d) { d.outerRadius = outerRadius - 10; });

    g.append("path")
      .attr("d", arc)
      .style("fill", function(d) { return color(d.data.Value); })
      .each((d) => { d.outerRadius = outerRadius - 10; })
      .on("mousemove", function(d) {
        d3.select(this)
          .style("filter", "url(#drop-shadow)");
        d3.select(this)
          .transition()
          .duration(500)
          .ease("bounce")
          .attr("transform", function(d){
            const dist = 1;
            d.midAngle = ((d.endAngle - d.startAngle)/2) + d.startAngle;
            const x = Math.sin(d.midAngle) * dist;
            const y = Math.cos(d.midAngle) * dist;
            return "translate(" + x + "," + y + ")";
          });
        const mousePos = d3.mouse(divNode);
        d3.select(this).transition().duration(200).delay(0).attrTween("d", function(d) {
          const i = d3.interpolate(d.outerRadius, outerRadius);
          return function(t) { d.outerRadius = i(t); return arc(d, 0); };
        });
      })


      .on("mouseover", function(d) {
        d3.select("pattern image")
          .attr("id", "myData")
          .style("visibility", "visible")
          .attr("href", d.data.image);
        svg.select("circle.image")
          .attr("fill", "url(#image)")
          .style("stroke", "#975EEE")
          .style("stroke-width", "3px");
        g.append("text").transition()
          .attr("class", "value-text")
          .attr("id", "myData")
          .text("Eth: " + `${d.data.Amount}`)
          .attr("text-anchor", "middle")
          .attr("dy", ".6em");
        g.append("text")
          .attr("class", "name-text")
          .attr("id", "myData")
          .text("Price: " + `${d.data.Price}`)
          .attr("text-anchor", "middle")
          .attr("dy", "2em");
        g.append("text")
          .attr("class", "price-text")
          .attr("id", "myData")
          .text("24h: " + `${d.data.Change}`)
          .attr("text-anchor", "middle")
          .attr("dy", "3.2em");
      })

      .on("mouseout", function(d){
        d3.selectAll("#myData")
          .style("visibility", "hidden");
        g.selectAll("#myData").remove();
        d3.select(this)
          .attr("stroke", "none")
          .style("filter", "none");
        d3.select(this)
          .transition()
          .duration(500)
          .ease("bounce")
          .attr("transform", "translate(0,0)");

        d3.select(this).transition().duration(200).delay(0).attrTween("d", function(d) {
          const i = d3.interpolate(d.outerRadius, outerRadius - 10);
          return function(t) { d.outerRadius = i(t); return arc(d, 0); };
        });
      })

      .on("click", function() {
        d3.select(this).transition().duration(200).delay(0).attrTween("d", function(d) {
          const i = d3.interpolate(d.outerRadius, outerRadius);
          return function(t) { d.outerRadius = i(t); return arc(d, 0); };
        });
      })

      .on("dblclick", function() {
        d3.select(this).transition().duration(200).delay(0).attrTween("d", function(d) {
          const i = d3.interpolate(d.outerRadius, outerRadius - 10);
          return function(t) { d.outerRadius = i(t); return arc(d, 0); };
        });
      });
  }
}
